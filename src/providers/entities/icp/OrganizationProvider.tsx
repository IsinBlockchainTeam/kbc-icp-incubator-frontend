...
import { Organization, OrganizationDriver, OrganizationParams, OrganizationService } from '@kbc-lib/coffee-trading-management-lib';
...
export type OrganizationContextState = {
    ...
    inviteOrganization: (email: string, name: string) => Promise<void>;
    loadData: () => Promise<void>;
};

export const OrganizationContext = createContext<OrganizationContextState>({} as OrganizationContextState);
...
export function OrganizationProvider(props: { children: ReactNode }) {
    ...
    const organizationService = useMemo(() => new OrganizationService(new OrganizationDriver(identity, entityManagerCanisterId)), [identity]);
    ...
    const storeOrganization = async (params: OrganizationParams) => {
        await writeTransaction(() => organizationService.createOrganization(params), ORGANIZATION_MESSAGE.SAVE);
    };

    const updateOrganization = async (ethAddress: string, params: OrganizationParams) => {
        await writeTransaction(() => organizationService.updateOrganization(ethAddress, params), ORGANIZATION_MESSAGE.UPDATE);
    };

    const inviteOrganization = async (email: string, name: string) => {
        await handleICPCall(async () => {
            await organizationService.inviteOrganization(email, name);
            openNotification('Success', ORGANIZATION_MESSAGE.INVITE.OK, NotificationType.SUCCESS, NOTIFICATION_DURATION);
        }, ORGANIZATION_MESSAGE.INVITE.LOADING);
    };
