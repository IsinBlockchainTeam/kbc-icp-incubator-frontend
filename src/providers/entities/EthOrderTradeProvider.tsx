...
export function EthOrderTradeProvider(props: { children: ReactNode }) {
    ...
    //                 ? getCompany(detailedOrderTrade.trade.commissioner).legalName
    //                 : getCompany(detailedOrderTrade.trade.supplier).legalName;
    ...
    return (
        <EthOrderTradeContext.Provider
            value={{
                detailedOrderTrade: null,
                getOrderTradeService: (address: string): OrderTradeService => {
                    return {} as OrderTradeService;
                },
                saveOrderTrade: async (orderTradeRequest: OrderTradeRequest): Promise<void> => {},
                updateOrderTrade: async (orderTradeRequest: OrderTradeRequest): Promise<void> => {},
                confirmNegotiation: async (): Promise<void> => {},
                notifyExpiration: async (email: string, message: string): Promise<void> => {},
                getNegotiationStatusAsync: async (orderId: number): Promise<NegotiationStatus> => {
                    return {} as NegotiationStatus;
                },
                getSupplierAsync: async (orderId: number): Promise<string> => {
                    return '';
                },
                getCustomerAsync: async (orderId: number): Promise<string> => {
                    return '';
                },
                getDetailedTradesAsync: async (): Promise<DetailedOrderTrade[]> => {
                    return [];
                }
            }}>
            {props.children}
