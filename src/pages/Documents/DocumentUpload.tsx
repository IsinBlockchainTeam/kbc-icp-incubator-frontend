...
import { EvaluationStatus, DocumentInfo, DocumentType } from '@kbc-lib/coffee-trading-management-lib';
import React, { useMemo } from 'react';
import { Flex, Tooltip } from 'antd';
import { WarningOutlined } from '@ant-design/icons';

type Props = {
    // TODO: change DocumentType with generic DocumentType and DocumentInfo with DocumentInfo, so that this component can be used for every document upload
    documentTypes: DocumentType[];
    oldDocumentsInfo: DocumentInfo[];
    onSubmit: (documentType: DocumentType, documentReferenceId: string, filename: string, fileContent: Blob) => Promise<void>;
...
export const DocumentUpload = (props: Props) => {
            ...
            },
            {
                ...
                optionRender: ({ data }) => (
                    <Flex justify="space-between">
                        ...
                        {oldDocumentsInfo.findIndex((d) => d.documentType === data.value) !== -1 && (
                            <Tooltip
                                placement="right"
                                title={
                                    <span>
                                        ...
                                        You can upload again to replace it, until it is not approved by the counterparty.
                                    </span>
                                }>
    ...
    return (
        <GenericForm
            ...
            elements={elements}
            submittable={true}
            onSubmit={(values) => onSubmit(values['documentType'], values['referenceId'], values['document'].name, values['document'])}
        />
    );
};
