...
jest.mock('../../../../../api/services/EthDocumentService');
...
describe('tradeView', () => {
    ...
    it('should get trade info and documents', async () => {
        mockTradeService.getTradeByIdAndType.mockReturnValue(
            new TradePreviewPresentable(1, [], 'supplier', TradeType.BASIC)
        );
        const mockGetDocumentsByTransactionIdAndType = jest
            .fn()
            .mockReturnValue(new DocumentPresentable(1, 'test document', 'application/pdf'));
        (EthDocumentService as jest.Mock).mockReturnValue({
            getDocumentsByTransactionIdAndType: mockGetDocumentsByTransactionIdAndType
        });
        ...
        await waitFor(() =>
            expect(mockTradeService.getTradeByIdAndType).toHaveBeenCalledWith(1, 1)
        );
        await waitFor(() =>
            expect(mockGetDocumentsByTransactionIdAndType).toHaveBeenCalledTimes(1)
        );
    ...
    it('should not load values if subjectClaims are not defined', () => {
        ...
        expect(openNotification).toHaveBeenCalledWith(
            'Error',
            'No information about company storage',
